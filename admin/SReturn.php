...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($bookId !== null) {
        ...
        if ($result->num_rows > 0) {
            ...
            $updateStmt->bind_param("ii", $newQuantity, $item['book_id']);
            $updateStmt->execute();
            $updateStmt->close();

            // Update the status to "return" in the borrow table
            $updateBorrowSql = "UPDATE borrow SET status = 'Returned' WHERE id = ?";
    ...
    // Fetch book details using prepared statement before displaying the form
    $bookId = $_GET['id'] ?? null;
